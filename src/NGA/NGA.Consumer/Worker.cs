...
namespace NGA.Consumer
{
    class Worker : BaseTask
    {
        ...
        public Worker(IServiceScopeFactory scopeFactory, IRabbitMQService rabbitMQService, ILogger<JfYuHttpRequest> logger1) : base(scopeFactory, rabbitMQService, logger1)
        {
        ...
        protected async Task<bool> HandleTopicAsync(Topic data)
        {
            ...
            Console.WriteLine($"{data.Tid}:{data.Title}开始");
            var reptileNum = 0;
            if (ConsumerType == "New")
            ...
            {
                Console.WriteLine($"错误:{ex.Message},{ex.InnerException?.Message}");
                var _log = new Log
                {
            ...
            finally
            {
                ...
                Console.WriteLine($"{data.Tid}:{data.Title}结束");
        ...
        protected async Task<Tuple<int, bool>> MainAsync(Topic t, int page)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(html))
                        continue;
                    if (html == null || html.Contains("帖子发布或回复时间超过限制") || html.Contains("302 Found") || html.Contains("帖子被设为隐藏") || html.Contains("查看所需的权限/条件"))
                    {
                        Console.WriteLine($"{t.Title}被隐藏");
            ...
            if (userInfoAll == null)
            {
                Console.WriteLine($"{t.Title},{page}:找不到用户信息");
                return new Tuple<int, bool>(0, true);
            }
            ...
            for (int i = 0; i < lou.Count; i++)
            {
                try
                {
                    ...
                    //获取replayid
                    if (sort != 0)
                        or.Pid = lou[i].SelectSingleNode(".//a[contains(@id,'pid')]").Id.Replace("pid", "").Replace("Anchor", "");
                    GetContextData(or, htmlDocument, contentNode.ChildNodes[4].InnerHtml);
                    GetFloorData(or, htmlDocument);
                    await GetUserInfo(or.Uid);
                    if (or.Uid.StartsWith("-1"))
                    {
                        UserinfoJson uij = null;
                        var a = anonymous.TryGetValue(or.Uid, out uij);
                        if (uij != null)
                        {
                            or.UName = GetName(uij.Username);
                        }
                    }
                    var oldhis = await _replayService.GetOneAsync(x => x.Sort == sort && x.Tid == t.Tid);
                    if (oldhis == null)
                    {
                        await _replayService.AddAsync(or);
                    }
                    else
                    {
                        or.Id = oldhis.Id;
                        if (oldhis.Content != or.Content)
                            await _replayHisService.AddAsync(new ReplayHis() { Pid = or.Pid, Tid = or.Tid, PostDate = or.PostDate, Uid = or.Uid, Content = or.Content });
                        await _replayService.UpdateAsync(or);
                    }
                    lastsort = or.Sort;
                }
                catch (Exception ex)
        ...
        protected async Task GetUserInfo(string uid)
        {
            ...
            if (user == null || (DateTime.Now - user.UpdatedTime).Days > 7)
            {
                ...
                try
                {
                    ...
                    var tempuser = await _userService.GetOneAsync(q => q.Uid == uid);
                    if (tempuser == null)
